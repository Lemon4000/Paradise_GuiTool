...
python -m PyInstaller $mode --noconsole --icon "ICON.png" -n $Name gui/main.py --add-data "config;config" --clean

if ($OneFile) {
  $distDir = "dist"
  # Rebuild with local runtime tmp dir to avoid embedded interpreter startup issues
  python -m PyInstaller -F --noconsole --icon "ICON.png" -n $Name gui/main.py --add-data "config;config" --runtime-tmpdir . --clean
